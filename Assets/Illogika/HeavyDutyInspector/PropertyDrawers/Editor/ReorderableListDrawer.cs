...
namespace HeavyDutyInspector
{
	...
	public class ReorderableListDrawer : IllogikaDrawer {
		...
		public override void OnGUI (Rect position, SerializedProperty prop, GUIContent label)
		{
			...
			if(index != 0)
			{
				if(GUI.Button(basePosition, "+", "ButtonLeft"))
				{
			...
			else
			{
				...
				GUI.Box(basePosition, "+", "ButtonLeft");
				GUI.color = temp;
			}
			...
			{
				if(GUI.Button(basePosition, "-", "ButtonRight") && index != list.Count - 1)
				{
			...
			else
			{
				Color temp = GUI.color;
				GUI.color = Color.gray;
				GUI.Box(basePosition, "-", "ButtonRight");
				GUI.color = temp;
			}
